...
function subArraySum(boards, workLoad, k) {
    let painters = 1;
    let currentSum = 0;
    
    for (let i = 0; i < boards.length; i++) {
        if (boards[i] > workLoad) return false;

        if (currentSum + boards[i] > workLoad) {
            painters++;
            currentSum = boards[i];
        }else{
            currentSum = currentSum + boards[i];
        }
    }
...
function findPaintersPartition(boards, k) {
    let start = Math.max(...boards);
    let end = boards.reduce((acc, sum) => sum + acc, 0);
    let ans = end;

    while (start <= end) {
        let mid = Math.floor((start+end)/2);

        let isPossible = subArraySum(boards,mid, k);
...
console.log(findPaintersPartition(boards, k));